...
class Actor(Network):
    ...
    def forward(self, states, visualize=False):
        assert states.shape[1] == self.num_scan + self.extra_inputs, \
            f"[Actor] Input shape mismatch: got {states.shape[1]}, expected {self.num_scan + self.extra_inputs}"

        scan = states[:, :self.num_scan].unsqueeze(1)  # [B, 1, 580]
        extra = states[:, self.num_scan:self.num_scan + self.extra_inputs]  # [B, 23]

        x = self.scan_conv(scan)
        x = torch.flatten(x, start_dim=1)
        x = torch.cat([x, extra], dim=1)

        x1 = torch.relu(self.fa1(x))
...
class Critic(Network):
    def __init__(self, name, state_size, action_size, hidden_size):
        ...
        self.num_scan = 580  #580 640 960    pi  1080  
        self.extra_inputs = 4
    ...
    def extract_features(self, states):
        assert states.shape[1] == self.num_scan + self.extra_inputs, \
            f"[Critic] Input shape mismatch: got {states.shape[1]}, expected {self.num_scan + self.extra_inputs}"
...
class Customed_TD3(OffPolicyAgent):
